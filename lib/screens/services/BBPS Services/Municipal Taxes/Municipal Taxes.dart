...
class _MunicipalTaxesScreenState extends State<MunicipalTaxesScreen> with TickerProviderStateMixin {
  // ─── Theme Colors ────────────────────────────────────────────────────────
  ...
  static Color _colorFor(String code, String label) {
    ...
    if (k.contains('delhi') || k.contains('mcd') || k.contains('ndmc')) return const Color(0xFF7C3AED);
    if (k.contains('mumbai') || k.contains('mcgm') || k.contains('bmc') || k.contains('thane') || k.contains('kalyan') || k.contains('kdmc')) return const Color(0xFF2563EB);
    if (k.contains('chennai') || k.contains('gcc')) return const Color(0xFF059669);
    if (k.contains('hyderabad') || k.contains('ghmc')) return const Color(0xFF4F46E5);
    if (k.contains('bengaluru') || k.contains('bbmp')) return const Color(0xFF0284C7);
    if (k.contains('kolkata') || k.contains('kmc')) return const Color(0xFF9333EA);
    if (k.contains('ahmedabad') || k.contains('amc') || k.contains('surat')) return const Color(0xFFD97706);
    if (k.contains('pune') || k.contains('pmc') || k.contains('pcmc')) return const Color(0xFFDB2777);
    if (k.contains('gram') || k.contains('panchayat')) return const Color(0xFF16A34A);
  ...
  static IconData _iconFor(String code, String label) {
    ...
    if (k.contains('property') || k.contains('land')) return Icons.home_work_rounded;
    if (k.contains('trade') || k.contains('license')) return Icons.store_rounded;
    if (k.contains('water') || k.contains('sewer')) return Icons.water_drop_rounded;
    ...
    try {
      ...
      if (data['success'] == true && data['operators'] != null && (data['operators'] as List).isNotEmpty) {
        ...
        if (mounted) {
          setState(() {
            _operators = [];
            _opsLoading = false;
            _opsError = data['message']?.toString() ?? 'No municipal authorities available. Please try again.';
          });
        }
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _operators = [];
          _opsLoading = false;
          _opsError = 'Failed to load municipal authorities. Please check your connection and retry.';
        });
      }
    }
  }
  ...
  Future<void> _onFetchBill() async {
    ...
    if (account.isEmpty) {
      _snack('Please enter your Assessment / Property / Consumer Number', isError: true);
      return;
    }
    if (_selectedOp == null) {
      _snack('Please select a municipal corporation', isError: true);
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        if (bill.isSuccess && bill.dueAmount > 0) {
          _snack('Tax bill fetched successfully! Amount: ₹${bill.dueAmount.toStringAsFixed(2)}');
      ...
      if (mounted) {
        ...
        _snack('Error fetching tax bill: $e', isError: true);
      }
    }
  }
  ...
  Future<void> _handleProceed() async {
    ...
    if (_selectedOp == null) { _snack('Please select a municipal corporation', isError: true); return; }
    ...
    _razorpayService.openPaymentGateway(
      amount: amount,
      description: 'Municipal Tax – ${_selectedOp!['label'] ?? 'Payment'}',
      name: 'DZI Infinity',
      onSuccess: (PaymentSuccessResponse response) {
        _doSubmitMunicipalTaxes(auth: auth, razorpayPaymentId: response.paymentId ?? '');
      },
  ...
  Future<void> _doSubmitMunicipalTaxes({required dynamic auth, required String razorpayPaymentId}) async {
    setState(() { _submitting = true; _resultStatus = null; });
    try {
      final res = await ApiService.postApi('/municipal-taxes/pay', {
        'user_id':             auth.userId,
      ...
      if (mounted) {
        setState(() {
          ...
          _resultMessage = (data['message']?.toString() ?? (_resultStatus == 'success' ? 'Municipal tax paid successfully!' : 'Payment failed')).replaceAll('(TEST MODE)', '').replaceAll('(TEST MODE - no real money moved)', '').trim();
              ...
              Padding(
                ...
                child: Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Select Municipal Corporation',
                  ...
                  Container(
                    ...
                    child: const Icon(Icons.location_city_rounded, color: primaryPurple, size: 20),
                  ),
                ]),
              ),
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by city or municipal corporation…',
                ...
                child: _opsLoading
                    ? const Padding(
                        ...
                        child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          SizedBox(height: 14),
                          Text('Loading municipal authorities…', style: TextStyle(color: textMuted)),
                        ]),
                      )
                    : _opsError != null
                        ? Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              child: const Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.location_city_rounded, color: primaryPurple, size: 15),
                SizedBox(width: 5),
                Text('MUNICIPAL TAX', style: TextStyle(color: primaryPurple, fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
              ]),
            ),
            const SizedBox(height: 14),
            RichText(
              text: const TextSpan(
                ...
                children: [
                  ...
                  TextSpan(text: 'Municipal Tax', style: TextStyle(color: primaryPurple, fontWeight: FontWeight.w900, fontSize: 27)),
                ],
              ),
            ),
            const SizedBox(height: 10),
            Text(
              'Enter your Property Tax / Consumer\nNumber and select your municipal\ncorporation to proceed.',
            ...
            child: SizedBox(
              ...
              child: Image.asset(
                ...
                errorBuilder: (ctx, err, st) => Image.asset(
                  'assets/Muncipal Taxes.png',
                  fit: BoxFit.contain,
                  errorBuilder: (ctx2, err2, st2) => Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(color: primaryPurple.withValues(alpha: 0.1), shape: BoxShape.circle),
                    child: const Icon(Icons.location_city_rounded, size: 64, color: primaryPurple),
                  ),
                ),
              ),
            ),
  ...
  Widget _buildPillGrid() => Container(
    ...
    child: Row(children: [
      Expanded(child: _pillItem(Icons.badge_outlined, 'Consumer ID', 'Enter Details')),
      Container(width: 1, height: 38, color: const Color(0xFFF1F5F9)),
      Expanded(child: _pillItem(Icons.location_city_outlined, 'Corporation', 'All Authorities')),
      Container(width: 1, height: 38, color: const Color(0xFFF1F5F9)),
  ...
  // ─── Input Form Card (Consumer ID + Authority + Amount) ───────────────────
  Widget _buildFormCard() {
    final selLabel = _selectedOp?['label']?.toString();
    final selCode  = _selectedOp?['code']?.toString() ?? '';
    final selColor = _selectedOp != null ? _colorFor(selCode, selLabel!) : primaryPurple;
    final selIcon  = _selectedOp != null ? _iconFor(selCode, selLabel!) : Icons.location_city_outlined;

    return Column(children: [
      // 1. Consumer ID / Property Tax No. Card
      Container(
        width: double.infinity,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: 'Enter Property Tax / Consumer Number',
            ...
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Enter Property Tax / Consumer Number' : null,
          ),
        ]),
      ),
      ...
      // 2. Operators / Municipal Authority Selector Card
      Container(
        width: double.infinity,
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: const Color(0xFFF3E8FF), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.location_city_rounded, color: primaryPurple, size: 18),
            ),
            const SizedBox(width: 12),
            const Text('Municipal Authority', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textDark)),
          ]),
          const SizedBox(height: 14),
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Expanded(
                  child: Text(
                    _opsLoading ? 'Loading authorities…' : selLabel ?? 'Select Municipal Corporation',
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                      fontSize: 13.5,
          ...
          // Dynamic Tax Amount input (shown when account no & operator selected)
          if (_canShowAmount) ...[
            const SizedBox(height: 18),
            const Row(children: [
              Icon(Icons.currency_rupee_rounded, size: 18, color: primaryPurple),
              SizedBox(width: 8),
              Text('Tax Amount (₹)', style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w800, color: textDark)),
            ]),
            const SizedBox(height: 10),
            TextFormField(
  ...
  Widget _buildInstantSettlementCard() => Container(
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(
      color: cardWhite,
      borderRadius: BorderRadius.circular(24),
    ...
    child: Row(children: [
      ...
      Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 2),
          Text(
            'Direct settlement with municipal authorities. Quick and hassle-free.',
  ...
  // ─── Submit Button ("Fetch Tax & Proceed") ───────────────────────────────
        ...
        child: Container(
          ...
          child: _submitting
              ...
              : Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(
                    _canShowAmount ? 'Pay Municipal Tax' : 'Fetch Tax & Proceed',
  ...
  Widget _buildResult() {
    ...
    final title = isOk ? 'Tax Paid Successfully!' : isPen ? 'Payment Processing…' : 'Payment Failed';
    ...
    final receipt = BbpsReceiptModel(
      serviceCategory: 'Municipal Tax / Property Tax',
      operatorName: _selectedOp?['label']?.toString() ?? 'Municipal Corporation',
      accountNumber: _consumerIdCtrl.text.trim(),
      customerName: _fetchedBill?.customerName ?? '',
      merchantTxnId: _merchantTxnId ?? 'TAX${now.millisecondsSinceEpoch}',
      dateTimeStr: dtStr,
      amount: paidAmt,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(
            ...
            child: TextButton(
              ...
              child: const Text('Pay Another Tax', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
            ),
          ),
        ]),
      ),
