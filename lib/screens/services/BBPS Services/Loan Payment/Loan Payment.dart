...
class _LoanPaymentScreenState extends State<LoanPaymentScreen>
  ...
  static const Color primaryDarkOrange = Color(0xFF028090);
  ...
  static Color _colorFor(String code, String loanType) {
    ...
    if (k.contains('bank') || k.contains('dcb') || k.contains('au_bank') || k.contains('andhra')) return const Color(0xFF0284C7);
    if (k.contains('bajaj'))   return const Color(0xFFFF5722);
    ...
    if (k.contains('vehicle') || k.contains('cars24') || k.contains('auto') || k.contains('baid')) return const Color(0xFF6366F1);
    if (k.contains('micro') || k.contains('arohan') || k.contains('annapurna_mfi') || k.contains('chaitanya') || k.contains('agora')) return const Color(0xFFEC4899);
    if (k.contains('education') || k.contains('eduvanz') || k.contains('avanse')) return const Color(0xFF8B5CF6);
    if (k.contains('msme') || k.contains('capri_global') || k.contains('ambit')) return const Color(0xFFD97706);
    if (k.contains('nidhi') || k.contains('ajeevak') || k.contains('alfastar')) return const Color(0xFF14B8A6);
    if (k.contains('agri')) return const Color(0xFF059669);
    if (k.contains('adani'))   return const Color(0xFF0EA5E9);
    if (k.contains('aditya') || k.contains('birla')) return const Color(0xFF7C3AED);
  ...
  @override
  void dispose() {
    _loanAccCtrl.dispose(); _amountCtrl.dispose(); _searchCtrl.dispose(); super.dispose();
  }
  ...
  Future<void> _handleProceed() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _resultMessage = data['message']?.toString() ?? (_resultStatus == 'success' ? 'EMI paid!' : 'Payment failed');
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Container(padding: const EdgeInsets.all(8),
                    ...
                    child: const Icon(Icons.account_balance_wallet_rounded, color: primaryOrange, size: 20)),
                ]),
              ),
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.search_rounded, color: primaryOrange, size: 20),
              ...
              Flexible(
                child: _opsLoading
                    ? const Padding(padding: EdgeInsets.all(32),
                        child: Column(mainAxisSize: MainAxisSize.min, children: [
                          CircularProgressIndicator(color: primaryOrange, strokeWidth: 2.5),
                          SizedBox(height: 14),
                                ...
                                itemBuilder: (_, i) {
                                  ...
                                  return ListTile(
                                    ...
                                    subtitle: loanType.isNotEmpty
                                        ...
                                        : null,
                                    trailing: isSel
                                        ? const Icon(Icons.check_circle_rounded, color: primaryOrange, size: 22)
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(color: cardWhite, borderRadius: BorderRadius.circular(24),
          ...
          boxShadow: [BoxShadow(color: primaryOrange.withValues(alpha: 0.08), blurRadius: 10, offset: const Offset(0, 2))]),
        child: const Row(children: [
          Icon(Icons.verified_rounded, color: primaryOrange, size: 18),
          SizedBox(width: 6),
          Text('BBPS Assured', style: TextStyle(color: primaryOrange, fontSize: 13, fontWeight: FontWeight.w700)),
        ])),
    ]),
  );
  ...
  Widget _buildHero() => Container(
    ...
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [primaryOrange, primaryDarkOrange, accentNavy],
          begin: Alignment.topLeft, end: Alignment.bottomRight),
      borderRadius: BorderRadius.circular(28),
      boxShadow: [BoxShadow(color: primaryOrange.withValues(alpha: 0.3), blurRadius: 20, offset: const Offset(0, 10))]),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
          ...
          child: const Row(children: [
            ...
            SizedBox(width: 4),
            Text('LOAN EMI', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.8)),
          ])),
        Container(padding: const EdgeInsets.all(8),
      ...
      RichText(text: const TextSpan(
        ...
        children: [
          ...
          TextSpan(text: '(Loan EMI)', style: TextStyle(color: Color(0xFFBAE6FD), fontWeight: FontWeight.w900)),
        ])),
      const SizedBox(height: 8),
      Text('Enter your Loan Account Number and select your lender',
  ...
  Widget _buildPills() => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
      _pill(Icons.badge_rounded, 'Loan A/C No.'),
      _pill(Icons.account_balance_rounded, 'Lenders'),
  ...
  Widget _pill(IconData icon, String label) => Column(children: [
    Container(width: 44, height: 44,
        decoration: BoxDecoration(color: primaryOrange.withValues(alpha: 0.08), shape: BoxShape.circle),
  ...
  Widget _buildFormCard() {
    ...
    final selColor    = _selectedOp != null ? _colorFor(selCode, selLoanType) : const Color(0xFF94A3B8);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Loan Account No.
        const Row(children: [
          Icon(Icons.badge_outlined, size: 18, color: primaryOrange),
          SizedBox(width: 8),
          Text('Loan Account Number', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textDark)),
        ]),
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            hintText: 'Enter your Loan Account / Contract No.',
            hintStyle: const TextStyle(color: Color(0xFF94A3B8), fontSize: 14),
            filled: true, fillColor: inputFill,
            contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
            prefixIcon: const Icon(Icons.tag_rounded, color: primaryOrange, size: 20),
        ...
        // Operators
        const Row(children: [
          Icon(Icons.account_balance_wallet_rounded, size: 18, color: primaryOrange),
          SizedBox(width: 8),
        ...
        InkWell(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(
                _opsLoading ? 'Loading providers…' : selLabel ?? 'Select Loan Provider',
              ...
              Container(
                ...
                child: Text(_opsError != null ? 'Retry' : 'Choose',
                    style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: primaryOrange))),
            ]),
          ),
        ),
        ...
        if (_canShowAmount) ...[
          const SizedBox(height: 22),
          const Row(children: [
            Icon(Icons.currency_rupee_rounded, size: 18, color: primaryOrange),
            SizedBox(width: 8),
            Text('EMI Amount (₹)', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textDark)),
          ]),
          const SizedBox(height: 10),
          TextFormField(
            ...
            decoration: InputDecoration(
              ...
              filled: true, fillColor: inputFill,
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
              prefixIcon: const Icon(Icons.currency_rupee, color: primaryOrange, size: 20),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: borderColor, width: 1.2)),
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: primaryOrange, width: 2)),
              errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: Colors.redAccent, width: 1.2)),
              focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: Colors.redAccent, width: 2)),
          ...
          Wrap(spacing: 8, runSpacing: 8,
            children: [1000, 2500, 5000, 10000, 15000, 25000].map((amt) => InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: primaryOrange.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: primaryOrange.withValues(alpha: 0.25))),
                child: Text('₹$amt', style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.bold, color: primaryOrange))),
            )).toList()),
        ],
      ]),
    );
  ...
  Widget _buildSubmitBtn() => SizedBox(
    ...
    child: ElevatedButton(
      style: ElevatedButton.styleFrom(padding: EdgeInsets.zero,
          ...
          elevation: 6, shadowColor: primaryOrange.withValues(alpha: 0.35)),
      onPressed: _submitting ? null : _handleProceed,
      child: Ink(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: _submitting ? [Colors.grey.shade400, Colors.grey.shade500] : [primaryOrange, primaryDarkOrange],
        ...
        child: Container(alignment: Alignment.center,
          child: _submitting
              ...
              : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Text('Pay Loan EMI', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 0.3)),
                  SizedBox(width: 8),
  ...
  Widget _buildResult() {
    ...
    final title = isOk ? 'EMI Paid!' : isPen ? 'Processing…' : 'Payment Failed';
      ...
      child: Container(padding: const EdgeInsets.all(28),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(width: double.infinity,
            ...
            decoration: BoxDecoration(color: primaryOrange.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(16),
                border: Border.all(color: primaryOrange.withValues(alpha: 0.2))),
            child: Column(children: [
              const Text('Transaction Reference ID', style: TextStyle(fontSize: 11, color: textMuted)),
              const SizedBox(height: 6),
              Text(_merchantTxnId ?? '—', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: primaryOrange, letterSpacing: 0.5)),
            ])),
          ...
          SizedBox(width: double.infinity, height: 50,
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: primaryOrange, foregroundColor: Colors.white, elevation: 3,
  ...
  Widget _buildAssurance() => Container(
    ...
    child: Column(children: [
      _tile(0, Icons.bolt_rounded, const Color(0xFFF59E0B), 'Instant EMI Credit', 'Direct settlement with lender', '⚡ Real-time EMI credited directly to your loan account.'),
      ...
      const Divider(height: 1, indent: 64, endIndent: 20, color: Color(0xFFF1F5F9)),
      _tile(2, Icons.receipt_long_rounded, primaryOrange, 'Instant Receipt', 'Official loan EMI payment proof', '🧾 Download official BBPS receipt after EMI payment.'),
    ]),
  );
      ...
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Icon(isExp ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, color: primaryOrange, size: 22),
          ]),
          if (isExp) ...[
            const SizedBox(height: 10),
            Container(width: double.infinity, padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: primaryOrange.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(14),
                  border: Border.all(color: primaryOrange.withValues(alpha: 0.15))),
              child: Text(detail, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: primaryDarkOrange, height: 1.35))),
          ],
        ])));
  }
}
