...
class _LoanPaymentScreenState extends State<LoanPaymentScreen>
  ...
  Future<void> _onFetchBill() async {
    ...
    final account = _loanAccCtrl.text.trim();
    if (account.isEmpty) {
      _snack('Please enter your Loan Account Number', isError: true);
      return;
    }
    if (_selectedOp == null) {
      _snack('Please select a loan provider', isError: true);
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        if (bill.isSuccess && bill.dueAmount > 0) {
          _snack('Loan EMI fetched successfully! Amount: ₹${bill.dueAmount.toStringAsFixed(2)}');
      ...
      if (mounted) {
        ...
        _snack('Error fetching EMI: $e', isError: true);
      }
    }
  }
  ...
  Future<void> _doSubmitLoanPayment({required dynamic auth, required String razorpayPaymentId}) async {
    ...
    try {
      final res = await ApiService.postApi('/loan-payment/pay', {
        'user_id':             auth.userId,
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_loanAccCtrl.text.trim().isNotEmpty && _selectedOp != null) ...[
            const SizedBox(height: 16),
            BbpsFetchedBillCard(
              bill: _fetchedBill,
  ...
  Widget _buildResult() {
    ...
    final receipt = BbpsReceiptModel(
      serviceCategory: 'Loan Repayment / EMI',
      operatorName: _selectedOp?['label']?.toString() ?? 'Lender / Bank',
      accountNumber: _loanAccCtrl.text.trim(),
      customerName: _fetchedBill?.customerName ?? '',
      merchantTxnId: _merchantTxnId ?? 'LON${now.millisecondsSinceEpoch}',
      dateTimeStr: dtStr,
      amount: paidAmt,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(
            ...
            child: TextButton(
              ...
              child: const Text('Pay Another EMI', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
            ),
          ),
        ]),
      ),
