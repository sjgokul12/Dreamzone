...
class _HousingSocietyScreenState extends State<HousingSocietyScreen>
  ...
  static Color _colorFor(String name) {
    ...
    if (k.contains('residency') || k.contains('tower')) return const Color(0xFF2563EB);
    ...
    if (k.contains('heights') || k.contains('park') || k.contains('square')) return const Color(0xFF4F46E5);
  ...
  Future<void> _onFetchBill() async {
    ...
    if (account.isEmpty) {
      _snack('Please enter your Flat / Unit / Consumer Number', isError: true);
      return;
    }
    if (_selectedOp == null) {
      _snack('Please select a housing society', isError: true);
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        if (bill.isSuccess && bill.dueAmount > 0) {
          _snack('Maintenance bill fetched successfully! Amount: ₹${bill.dueAmount.toStringAsFixed(2)}');
      ...
      if (mounted) {
        ...
        _snack('Error fetching maintenance bill: $e', isError: true);
      }
    }
  }
  ...
  Future<void> _handleProceed() async {
    ...
    _razorpayService.openPaymentGateway(
      amount: amount,
      description: 'Housing Society – ${_selectedOp!['label'] ?? 'Payment'}',
      name: 'DZI Infinity',
      onSuccess: (PaymentSuccessResponse response) {
        _doSubmitHousingSociety(auth: auth, razorpayPaymentId: response.paymentId ?? '');
      },
  ...
  Future<void> _doSubmitHousingSociety({required dynamic auth, required String razorpayPaymentId}) async {
    setState(() { _submitting = true; _resultStatus = null; });
    try {
      final res = await ApiService.postApi('/housing-society/pay', {
        'user_id':             auth.userId,
      ...
      if (mounted) {
        setState(() {
          ...
          _resultMessage = (data['message']?.toString() ?? (_resultStatus == 'success' ? 'Maintenance paid successfully!' : 'Payment failed')).replaceAll('(TEST MODE)', '').replaceAll('(TEST MODE - no real money moved)', '').trim();
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Container(
                    ...
                    child: const Icon(Icons.apartment_rounded, color: primaryPurple, size: 20),
                  ),
                ]),
              ),
              Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              child: const Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.apartment_rounded, color: primaryPurple, size: 15),
                SizedBox(width: 5),
                Text('SOCIETY MAINTENANCE', style: TextStyle(color: primaryPurple, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
              ]),
            ),
            const SizedBox(height: 12),
            RichText(
              text: const TextSpan(
                ...
                children: [
                  ...
                  TextSpan(text: 'Society\nMaintenance', style: TextStyle(color: primaryPurple, fontWeight: FontWeight.w900, fontSize: 23)),
                ],
              ),
            ),
            const SizedBox(height: 8),
            Text(
              'Enter Flat/Consumer Number and select your Housing Society.',
              ...
              child: Image.asset(
                ...
                errorBuilder: (ctx, err, st) => Image.asset(
                  ...
                  errorBuilder: (ctx2, err2, st2) => Container(
                    ...
                    decoration: BoxDecoration(color: primaryPurple.withValues(alpha: 0.1), shape: BoxShape.circle),
                    child: const Icon(Icons.apartment_rounded, size: 64, color: primaryPurple),
                  ),
                ),
              ),
            ),
  ...
  Widget _buildPillGrid() => Container(
    ...
    child: Row(children: [
      Expanded(child: _pillItem(Icons.badge_outlined, 'Flat / ID No.', 'Enter Details')),
      Container(width: 1, height: 38, color: const Color(0xFFF1F5F9)),
      Expanded(child: _pillItem(Icons.apartment_outlined, 'Societies', 'All Societies')),
      Container(width: 1, height: 38, color: const Color(0xFFF1F5F9)),
  ...
  // ─── Input Form Card (Flat No + Society + Amount) ─────────────────────────
  Widget _buildFormCard() {
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Text('Flat No. / Consumer ID', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textDark)),
          ]),
          const SizedBox(height: 14),
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: 'Enter your Flat / Consumer ID',
            ...
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Enter Flat / Consumer ID' : null,
          ),
        ]),
      ),
      ...
      // 2. Operators / Housing Society Selector Card
      Container(
        width: double.infinity,
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: const Color(0xFFF3E8FF), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.apartment_rounded, color: primaryPurple, size: 18),
            ),
            const SizedBox(width: 12),
          ...
          // Dynamic Maintenance Amount input (shown when account no & operator selected)
          if (_canShowAmount) ...[
            const SizedBox(height: 18),
            const Row(children: [
              Icon(Icons.currency_rupee_rounded, size: 18, color: primaryPurple),
              SizedBox(width: 8),
              Text('Maintenance Amount (₹)', style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w800, color: textDark)),
            ]),
  ...
  Widget _buildHassleFreeCard() => Container(
    ...
    child: Row(children: [
      ...
      Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Hassle-free Payments', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: textDark)),
          const SizedBox(height: 2),
          Text(
            'Pay your society maintenance charges instantly and securely.',
  ...
  // ─── Submit Button ("Fetch & Proceed to Pay") ────────────────────────────
        ...
        child: Container(
          ...
          child: _submitting
              ...
              : Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(
                    _canShowAmount ? 'Pay Maintenance Bill' : 'Fetch & Proceed to Pay',
  ...
  Widget _buildResult() {
    ...
    final title = isOk ? 'Maintenance Paid Successfully!' : isPen ? 'Payment Processing…' : 'Payment Failed';
    ...
    final receipt = BbpsReceiptModel(
      serviceCategory: 'Housing Society Maintenance',
      operatorName: _selectedOp?['label']?.toString() ?? 'Housing Society',
      accountNumber: _consumerIdCtrl.text.trim(),
      customerName: _fetchedBill?.customerName ?? '',
      merchantTxnId: _merchantTxnId ?? 'HOU${now.millisecondsSinceEpoch}',
      dateTimeStr: dtStr,
      amount: paidAmt,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(
            ...
            child: TextButton(
              ...
              child: const Text('Pay Another Maintenance', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
            ),
          ),
        ]),
      ),
