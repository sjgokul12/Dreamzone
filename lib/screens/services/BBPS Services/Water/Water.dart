...
class _WaterScreenState extends State<WaterScreen> with TickerProviderStateMixin {
  ...
  static Color _colorFor(String code, String label) {
    ...
    if (k.contains('mumbai') || k.contains('pune') || k.contains('pimpri') || k.contains('pcmc') || k.contains('kalyan') || k.contains('kdmc')) return const Color(0xFF0284C7);
    if (k.contains('hyderabad') || k.contains('hmwssb') || k.contains('warangal')) return const Color(0xFF6366F1);
    if (k.contains('gujarat') || k.contains('ahmedabad') || k.contains('surat')) return const Color(0xFFF59E0B);
    if (k.contains('madhya') || k.contains('bhopal') || k.contains('indore') || k.contains('jabalpur') || k.contains('gwalior') || k.contains('ujjain')) return const Color(0xFF8B5CF6);
    if (k.contains('punjab') || k.contains('ludhiana') || k.contains('chandigarh') || k.contains('jalandhar') || k.contains('amritsar')) return const Color(0xFF059669);
    if (k.contains('haryana') || k.contains('gurugram')) return const Color(0xFFEA580C);
    if (k.contains('kerala') || k.contains('kwa')) return const Color(0xFF10B981);
    if (k.contains('rajasthan') || k.contains('bhiwadi')) return const Color(0xFFEC4899);
    if (k.contains('uttarakhand')) return const Color(0xFF0891B2);
    if (k.contains('ranchi') || k.contains('jharkhand') || k.contains('silvassa')) return const Color(0xFF14B8A6);
    return primaryBlue;
  }
  ...
  Future<void> _fetchOperators() async {
    ...
    try {
      ...
      if (data['success'] == true && data['operators'] != null) {
        ...
        if (mounted) {
          setState(() {
          _opsError = data['message']?.toString() ?? 'Failed to load water authorities';
          _opsLoading = false;
        });
        }
      }
  ...
  Future<void> _handleProceed() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _resultMessage = data['message']?.toString() ?? (_resultStatus == 'success' ? 'Water bill paid!' : 'Payment failed');
        _merchantTxnId = data['merchant_txn_id']?.toString() ?? 'WAT${DateTime.now().millisecondsSinceEpoch}';
      });
      }
    } catch (_) {
      if (mounted) {
        ...
        _merchantTxnId = 'WAT${DateTime.now().millisecondsSinceEpoch}';
      });
      }
    }
  }
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Container(padding: const EdgeInsets.all(8),
                    ...
                    child: const Icon(Icons.water_drop_rounded, color: primaryBlue, size: 20)),
                ]),
              ),
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by city or authority name…',
                ...
                child: _opsLoading
                    ? const Padding(padding: EdgeInsets.all(32),
                        child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          SizedBox(height: 14),
                          Text('Loading water authorities…', style: TextStyle(color: textMuted)),
                        ]))
                    : _opsError != null
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
          ...
          child: const Row(children: [
            Icon(Icons.water_drop_rounded, color: Colors.lightBlueAccent, size: 16),
            SizedBox(width: 4),
            Text('WATER BILL', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.8)),
          ])),
        Container(padding: const EdgeInsets.all(8),
          decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.2), shape: BoxShape.circle),
          child: const Icon(Icons.water_damage_rounded, color: Colors.white, size: 22)),
      ]),
      const SizedBox(height: 18),
      RichText(text: const TextSpan(
        ...
        children: [
          ...
          TextSpan(text: '(Water Bill)', style: TextStyle(color: Color(0xFFBAE6FD), fontWeight: FontWeight.w900)),
        ])),
      const SizedBox(height: 8),
      Text('Enter Consumer ID and select your water authority',
  ...
  Widget _buildPills() => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
      ...
      _pill(Icons.water_drop_rounded, 'Operators'),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: accentCyan, width: 1.2)),
            focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: accentCyan, width: 2)),
          ),
        ...
        // Operators
        const Row(children: [
          Icon(Icons.water_drop_rounded, size: 18, color: primaryBlue),
          SizedBox(width: 8),
            ...
            child: Row(children: [
              Container(padding: const EdgeInsets.all(8),
                ...
                child: _opsLoading
                    ...
                    : Icon(Icons.water_drop_rounded, color: selColor, size: 20)),
              const SizedBox(width: 12),
              Expanded(child: Text(
                _opsLoading ? 'Loading operators…' : selLabel ?? 'Select Water Authority',
        ...
        if (_opsError != null) ...[
          ...
          GestureDetector(onTap: _fetchOperators,
            child: Row(children: [
              const Icon(Icons.refresh_rounded, size: 14, color: accentCyan),
              const SizedBox(width: 4),
              Expanded(child: Text(_opsError!, maxLines: 2, overflow: TextOverflow.ellipsis,
                  style: const TextStyle(fontSize: 11, color: accentCyan))),
            ])),
        ],
        ...
        child: Container(alignment: Alignment.center,
          child: _submitting
              ...
              : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Text('Pay Water Bill', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 0.3)),
                  SizedBox(width: 8),
  ...
  Widget _buildAssurance() => Container(
    ...
    child: Column(children: [
      _tile(0, Icons.bolt_rounded, const Color(0xFFF59E0B), 'Instant Processing', 'Direct settlement with water authority', '⚡ Real-time bill settlement with your municipal water authority.'),
      ...
      const Divider(height: 1, indent: 64, endIndent: 20, color: Color(0xFFF1F5F9)),
      _tile(2, Icons.receipt_long_rounded, primaryBlue, 'Instant Receipt', 'Official proof of water bill payment', '🧾 Download official BBPS receipt after successful payment.'),
    ]),
  );
