...
class _ServiceDetailScreenState extends State<ServiceDetailScreen>
  ...
  Future<void> _fetchFormConfig() async {
    final rawId = widget.service['id'];
    final intId = rawId is int ? rawId : int.tryParse(rawId?.toString() ?? '');
    final nameKey = (widget.service['name'] ?? '').toString().toLowerCase();

    if (intId != null) {
      try {
        final data = await _api.getServiceForm(intId);
        if (data['success'] == true && mounted) {
          _formConfig = data['form'];
          _sections = data['sections'] ?? [];
        }
      } catch (e) {
        debugPrint('Error fetching service form from API: $e');
      }
    }

    if (_sections.isEmpty) {
      _applyBuiltInFallbackIfNeeded(nameKey, rawId);
    }
    ...
    if (intId != null) {
      try {
        final res = await http.get(
          Uri.parse(
            '${ApiService.baseUrl}/services/$intId/required-documents',
          ),
  ...
  void _applyBuiltInFallbackIfNeeded(String sName, dynamic rawId) {
    if (sName.contains('aadhaar') || sName.contains('aadhar')) {
      _sections = [
        {
          'id': 401,
          'section_name': 'Soft Copy',
          'description': 'Download or request digital E-Aadhaar PDF copy.',
          'fee': '₹30',
          'processing_time': 'Instant',
          'fields': [
            {'field_name': 'aadhaar_or_eid', 'field_label': 'Aadhaar Number or EID Number', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'full_name', 'field_label': 'Full Name as on Aadhaar', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'dob', 'field_label': 'Date of Birth (DD/MM/YYYY)', 'field_type': 'date', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Pincode', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Registered Mobile Number', 'field_type': 'phone', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_aadhaar_soft_proof', 'doc_name': 'Aadhaar Enrolment Slip / ID Proof', 'is_mandatory': 0},
          ]
        },
        {
          'id': 402,
          'section_name': 'Hard Copy',
          'description': 'Request physical PVC Aadhaar hard copy delivered to your address.',
          'fee': '₹100',
          'processing_time': '3-5 Days',
          'fields': [
            {'field_name': 'full_name', 'field_label': 'Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'aadhaar_number', 'field_label': 'Aadhaar Number (12 Digits)', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'delivery_address_1', 'field_label': 'Delivery Address Line 1', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'delivery_address_2', 'field_label': 'Delivery Address Line 2', 'field_type': 'text', 'is_required': 0},
            {'field_name': 'city', 'field_label': 'City / District', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'state', 'field_label': 'State', 'field_type': 'select', 'options': 'Karnataka, Tamil Nadu, Kerala, Andhra Pradesh, Telangana, Maharashtra, Other', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Pincode', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'address_proof_type', 'field_label': 'Address Proof Document Type', 'field_type': 'select', 'options': 'Electricity Bill, Water Bill, Voter ID, Passport, Rent Agreement, Ration Card', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_aadhaar_addr_proof', 'doc_name': 'Address Proof Document', 'is_mandatory': 1},
            {'id': 'doc_aadhaar_copy', 'doc_name': 'Aadhaar Card Copy', 'is_mandatory': 1},
          ]
        }
      ];
      ...
      _sections = [
        {
          'id': 201,
          'section_name': 'New PAN Card Application',
          'description': 'Form 49A application for fresh Indian resident PAN card.',
          'fee': '₹110',
          'processing_time': '7-10 Days',
          'fields': [
            {'field_name': 'applicant_type', 'field_label': 'Category of Applicant', 'field_type': 'select', 'options': 'Individual, Firm, Body of Individuals, Trust', 'is_required': 1},
            {'field_name': 'full_name', 'field_label': 'Full Name (First Middle Last)', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'father_name', 'field_label': 'Father\'s Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'dob', 'field_label': 'Date of Birth (DD/MM/YYYY)', 'field_type': 'date', 'is_required': 1},
            {'field_name': 'gender', 'field_label': 'Gender', 'field_type': 'select', 'options': 'Male, Female, Transgender', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Mobile Number', 'field_type': 'phone', 'is_required': 1},
            {'field_name': 'email', 'field_label': 'Email Address', 'field_type': 'email', 'is_required': 1},
            {'field_name': 'aadhaar_number', 'field_label': 'Aadhaar Number', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'address', 'field_label': 'Full Communication Address', 'field_type': 'textarea', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Pincode', 'field_type': 'number', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_pan_id_proof', 'doc_name': 'Identity Proof (Aadhaar / Voter ID)', 'is_mandatory': 1},
            {'id': 'doc_pan_addr_proof', 'doc_name': 'Address Proof (Electricity Bill / Bank Passbook)', 'is_mandatory': 1},
            {'id': 'doc_pan_photo', 'doc_name': 'Passport Size Photograph', 'is_mandatory': 1},
          ]
        },
        {
          'id': 202,
          'section_name': 'Correction / Update PAN',
          'description': 'Correct name, father\'s name, DOB, or address on existing PAN card.',
          'fee': '₹110',
          'processing_time': '7-10 Days',
          'fields': [
            {'field_name': 'existing_pan', 'field_label': 'Existing PAN Number', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'full_name', 'field_label': 'Applicant Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'correction_type', 'field_label': 'Correction Required', 'field_type': 'select', 'options': 'Name Correction, DOB Correction, Father Name Correction, Address Update, Photo/Signature Update', 'is_required': 1},
            {'field_name': 'corrected_value', 'field_label': 'New / Corrected Detail Value', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Mobile Number', 'field_type': 'phone', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_pan_existing_copy', 'doc_name': 'Existing PAN Card Copy', 'is_mandatory': 1},
            {'id': 'doc_pan_corr_proof', 'doc_name': 'Proof of Correction Document', 'is_mandatory': 1},
          ]
        },
        {
          'id': 203,
          'section_name': 'Foreign PAN Application',
          'description': 'Form 49AA application for foreign citizens, NRIs, or foreign entities.',
          'fee': '₹1020',
          'processing_time': '10-15 Days',
          'fields': [
            {'field_name': 'applicant_category', 'field_label': 'Category of Applicant', 'field_type': 'select', 'options': 'Foreign Citizen, Foreign Entity, Non-Resident Indian (NRI)', 'is_required': 1},
            {'field_name': 'full_name', 'field_label': 'Applicant / Entity Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'passport_tax_id', 'field_label': 'Passport Number / Foreign Tax ID', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'dob_incorporation', 'field_label': 'Date of Birth / Date of Incorporation', 'field_type': 'date', 'is_required': 1},
            {'field_name': 'country_citizenship', 'field_label': 'Country of Citizenship / Registration', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'International Contact Mobile Number', 'field_type': 'phone', 'is_required': 1},
            {'field_name': 'email', 'field_label': 'Email Address', 'field_type': 'email', 'is_required': 1},
            {'field_name': 'overseas_address', 'field_label': 'Overseas Residential / Business Address', 'field_type': 'textarea', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Zip Code / Pincode', 'field_type': 'text', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_foreign_passport', 'doc_name': 'Foreign Passport / Tax Identification Proof', 'is_mandatory': 1},
            {'id': 'doc_foreign_addr', 'doc_name': 'Overseas Address Proof', 'is_mandatory': 1},
            {'id': 'doc_foreign_photo', 'doc_name': 'Passport Size Photograph', 'is_mandatory': 1},
          ]
        }
      ];
      ...
      _sections = [
        {
          'id': 301,
          'section_name': 'GST Registration',
          'description': 'Register business to obtain new 15-digit GSTIN number.',
          'fee': '₹499',
          'processing_time': '5-7 Days',
          'fields': [
            {'field_name': 'business_name', 'field_label': 'Legal Name of Business', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'trade_name', 'field_label': 'Trade Name (If any)', 'field_type': 'text', 'is_required': 0},
            {'field_name': 'constitution', 'field_label': 'Constitution of Business', 'field_type': 'select', 'options': 'Proprietorship, Partnership, Private Limited, LLP, One Person Company', 'is_required': 1},
            {'field_name': 'proprietor_pan', 'field_label': 'PAN of Proprietor / Business', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Primary Mobile Number', 'field_type': 'phone', 'is_required': 1},
            {'field_name': 'email', 'field_label': 'Primary Email Address', 'field_type': 'email', 'is_required': 1},
            {'field_name': 'business_address', 'field_label': 'Principal Place of Business Address', 'field_type': 'textarea', 'is_required': 1},
            {'field_name': 'state', 'field_label': 'State', 'field_type': 'select', 'options': 'Karnataka, Tamil Nadu, Kerala, Andhra Pradesh, Telangana, Maharashtra, Other', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Pincode', 'field_type': 'number', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_gst_pan', 'doc_name': 'PAN Card of Business / Proprietor', 'is_mandatory': 1},
            {'id': 'doc_gst_aadhaar', 'doc_name': 'Aadhaar Card of Proprietor / Partners', 'is_mandatory': 1},
            {'id': 'doc_gst_addr', 'doc_name': 'Business Address Proof (Electricity Bill / Rental Agreement)', 'is_mandatory': 1},
            {'id': 'doc_gst_bank', 'doc_name': 'Cancelled Cheque / Bank Passbook', 'is_mandatory': 1},
          ]
        }
      ];
      ...
      _sections = [
        {
          'id': 101,
          'section_name': 'New Voter ID Application',
          'description': 'Form 6 application for fresh Voter ID enrollment.',
          'fee': '₹50',
          'processing_time': '15-20 Days',
          'fields': [
            {'field_name': 'full_name', 'field_label': 'Applicant Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'relative_name', 'field_label': 'Father\'s / Mother\'s / Husband\'s Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'relation_type', 'field_label': 'Relation Type', 'field_type': 'select', 'options': 'Father, Mother, Husband, Other', 'is_required': 1},
            {'field_name': 'gender', 'field_label': 'Gender', 'field_type': 'select', 'options': 'Male, Female, Third Gender', 'is_required': 1},
            {'field_name': 'dob', 'field_label': 'Date of Birth (DD/MM/YYYY)', 'field_type': 'date', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Mobile Number', 'field_type': 'phone', 'is_required': 1},
            {'field_name': 'state', 'field_label': 'State', 'field_type': 'select', 'options': 'Karnataka, Tamil Nadu, Kerala, Andhra Pradesh, Telangana, Maharashtra, Other', 'is_required': 1},
            {'field_name': 'district', 'field_label': 'District', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'constituency', 'field_label': 'Assembly Constituency', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'full_address', 'field_label': 'Full Residential Address', 'field_type': 'textarea', 'is_required': 1},
            {'field_name': 'pincode', 'field_label': 'Pincode', 'field_type': 'number', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_voter_photo', 'doc_name': 'Passport Size Color Photograph', 'is_mandatory': 1},
            {'id': 'doc_voter_age', 'doc_name': 'Age Proof (Birth Cert / SSLC / PAN / Aadhaar)', 'is_mandatory': 1},
            {'id': 'doc_voter_addr', 'doc_name': 'Address Proof (Ration Card / Electricity Bill)', 'is_mandatory': 1},
          ]
        },
        {
          'id': 102,
          'section_name': 'Voter ID Correction / Update',
          'description': 'Form 8 application for voter card corrections or address transfer.',
          'fee': '₹50',
          'processing_time': '10-15 Days',
          'fields': [
            {'field_name': 'epic_number', 'field_label': 'Voter ID / EPIC Number', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'full_name', 'field_label': 'Applicant Full Name', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'correction_type', 'field_label': 'Field to Correct', 'field_type': 'select', 'options': 'Name Correction, Address Change, Photo Replacement, DOB Correction', 'is_required': 1},
            {'field_name': 'corrected_value', 'field_label': 'Corrected Detail Value', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'mobile', 'field_label': 'Mobile Number', 'field_type': 'phone', 'is_required': 1},
          ],
          'documents': [
            {'id': 'doc_voter_epic_copy', 'doc_name': 'Existing Voter ID Copy', 'is_mandatory': 1},
            {'id': 'doc_voter_proof', 'doc_name': 'Supporting Proof Document', 'is_mandatory': 1},
          ]
        },
        {
          'id': 103,
          'section_name': 'Download E-Voter Card',
          'description': 'Download digital e-EPIC copy.',
          'fee': '₹20',
          'processing_time': 'Instant',
          'fields': [
            {'field_name': 'epic_number', 'field_label': 'EPIC Number / Form Ref Number', 'field_type': 'text', 'is_required': 1},
            {'field_name': 'state', 'field_label': 'State', 'field_type': 'select', 'options': 'Karnataka, Tamil Nadu, Kerala, Andhra Pradesh, Telangana, Maharashtra, Other', 'is_required': 1},
            {'field_name': 'registered_mobile', 'field_label': 'Registered Mobile Number', 'field_type': 'phone', 'is_required': 1},
          ],
          'documents': []
        }
      ];
      ...
      _sections = [
        {
          'id': 501,
          'section_name': 'Cash Withdrawal',
          'description': 'AEPS Aadhaar biometric cash withdrawal service.',
          'fee': 'Free',
          'processing_time': 'Instant',
          'fields': [
            {'field_name': 'customer_aadhaar', 'field_label': 'Customer Aadhaar Number (12 Digits)', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'bank_name', 'field_label': 'Select Bank', 'field_type': 'select', 'options': 'State Bank of India, HDFC Bank, ICICI Bank, Axis Bank, Canara Bank, Union Bank, Bank of Baroda, Punjab National Bank, Other', 'is_required': 1},
            {'field_name': 'withdrawal_amount', 'field_label': 'Withdrawal Amount (₹)', 'field_type': 'number', 'is_required': 1},
            {'field_name': 'customer_mobile', 'field_label': 'Customer Mobile Number', 'field_type': 'phone', 'is_required': 1},
          ],
          'documents': []
        },
        {
          'id': 502,
          'section_name': 'Balance Enquiry',
          'description': 'Check bank account balance using Aadhaar authentication.',
          'fee': 'Free',
          'processing_time': 'Instant',
          'fields': [
            {'field_name': 'customer_aadhaar', 'field_label': 'Customer Aadhaar Number (12 Digits)', 'field_type': 'number', 'is_required': 1},
        ...
        {
          'id': 503,
          'section_name': 'Mini Statement',
          'description': 'Retrieve last 10 transactions mini statement via AEPS.',
          'fee': 'Free',
          'processing_time': 'Instant',
          'fields': [
            {'field_name': 'customer_aadhaar', 'field_label': 'Customer Aadhaar Number (12 Digits)', 'field_type': 'number', 'is_required': 1},
  ...
  void _useSavedDetails() {
    ...
    setState(() {
      ...
      if (_controllers.containsKey('dob') &&
          _controllers['dob']!.text.isNotEmpty) {
      ...
      if (docs.isNotEmpty && _savedDocuments.isNotEmpty) {
        for (var doc in docs) {
          ...
          for (var savedDoc in _savedDocuments) {
            final savedDocType = (savedDoc['doc_type'] ?? '')
                .toString()
                .toLowerCase();
            final savedDocName = (savedDoc['doc_name'] ?? '')
                .toString()
                .toLowerCase();
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            '✨ Saved details applied! Documents auto-attached!',
          ),
  ...
  Future<void> _fetchAndAttachDocument(
    String filePath,
    String fileName,
    String docId,
  ) async {
  ...
  bool _isFieldVisible(Map<String, dynamic> field) {
    ...
    if (parentId == null ||
        showWhen == null ||
        showWhen.toString().trim().isEmpty) {
      return true;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _submitted
          ? _buildSuccessScreen(name)
          : _buildDetailScreen(name, color, icon),
    );
  }
              ...
              : FadeTransition(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _sectionTitle(
                        _formConfig?['form_title'] ?? 'Application Form',
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                ..._sections.asMap().entries.map((entry) {
                                  ...
                                  final time =
                                      section['processing_time'] ?? '3-7 Days';
      ...
      child: Column(
        ...
        children: [
          ...
          ...docs.map((doc) {
            ...
            final mandatory =
                doc['is_mandatory'] == 1 || doc['is_mandatory'] == true;
            final files = _uploadedDocs[docId] ?? [];
          ...
          Row(
            children: [
              Icon(
                hasFiles
                    ...
                    : (mandatory
                          ...
                          : Icons.info_outline),
                color: hasFiles ? success : (mandatory ? warning : Colors.grey),
                size: 22,
              ),
              const SizedBox(width: 8),
              Expanded(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: mandatory ? warning.withAlpha(25) : Colors.grey[200]!,
          ...
          child: Column(
            ...
            children: [
              ...
              if (selectedFields.isNotEmpty)
                _buildGroupedFields(selectedFields),
  ...
  Widget _buildAddressSaveReuseRow(String fieldName) {
    ...
    final savedAddress = (_savedDetails?['address_line1'] ?? '')
        .toString()
        .trim();
    ...
    switch (type) {
      ...
      case 'select':
        ...
        return DropdownButtonFormField<String>(
          initialValue: (cv.isEmpty || !options.contains(cv)) ? null : cv,
          decoration: inputDecoration,
          isExpanded: true,
          items: options
              .map(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(
              ...
              borderSide: BorderSide(
                color: formFieldState.hasError ? Colors.red : Colors.grey[300]!,
              ),
            ),
            filled: true,
            fillColor: Colors.grey[50],
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.check_circle, color: success, size: 80),
            ),
